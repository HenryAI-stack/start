...
- **Tile icons are fetched from a fixed path: `https://henryai-stack.github.io/<repo>/favicon.ico`** (repo *deployed root*, not `src/`, not `icons/`). A tool that keeps its favicon anywhere else — or only as an inline SVG / emoji data-URI in its own `<head>` — will show the two-letter monogram fallback instead. To give a tool a real tile icon, put a `favicon.ico` at its Pages root: commit it to the repo root for plain-static projects, or to `public/favicon.ico` for the Vite projects (`people-os`, `recruit-os`), since Vite copies `public/` to the build root. Nothing in this repo needs to change.
